...
function renderConquistas(filter = 'all') {
  ...
  container.innerHTML = filtered
    .map((item) => {
      ...
      if (item.xp)
        rewardBadges.push(
          `<span class="reward-badge reward-xp">+${item.xp} XP</span>`
        );

      if (item.titleReward)
        rewardBadges.push(
          `<span class="reward-badge reward-titulo">${item.titleReward}</span>`
        );
      ...
      return `
        <div class="conquista-card ${badgeClass} ${specialClass}">
          ...
          <div class="conquista-completers">
            ${
              completed
                ? `Concluída por ${item.completedBy.length} personagem(es)`
                : 'Disponível para conquistar'
            }
          </div>
        </div>
      `;
